...
/**
 * Build the "last 5 years" list for the Year filter chip. Anchored to
 * the current calendar year at render time so it stays current without
 * any config updates.
 */
const buildYearOptions = (): number[] => {
    const now = new Date().getFullYear();
    return [0, 1, 2, 3, 4].map((offset) => now - offset);
};

/**
 * Whitelist ``<mark>`` / ``</mark>`` in the Postgres ``ts_headline``
 * output and strip everything else. The server hands us an HTML
 * fragment generated by Postgres — the only tags it is configured to
 * emit are the ``<mark>`` pair — but we belt-and-braces sanitise here
 * so a hostile abstract string that happened to contain angle-bracket
 * content can't reach ``dangerouslySetInnerHTML`` untouched.
 */
const sanitiseHighlight = (html: string): string => {
    if (!html) return '';
    // Kill anything that looks like a tag other than the two we allow.
    // We compare on a lowercased match to catch ``<MARK>`` etc.
    return html.replace(/<\/?([a-zA-Z][a-zA-Z0-9]*)(\s[^>]*)?>/g, (full, name: string) => {
        const lower = (name || '').toLowerCase();
        if (lower === 'mark') {
            // Rebuild without attributes so no ``<mark onclick=...>``
            // slips through even if the server ever changes shape.
...
const SearchPage: React.FC = () => {
    ...
    const initialYearRaw = params.get('year');
    const initialYear = initialYearRaw ? parseInt(initialYearRaw, 10) || 0 : 0;
    const initialCategory = params.get('category') || '';

    const [q, setQ] = useState(initialQ);
    const [kind, setKind] = useState<SearchKind>(initialKind);
    const [page, setPage] = useState<number>(initialPage);
    // A ``year`` of 0 (or ``NaN``) means "All years" — we skip
    // emitting the URL/query param in that case.
    const [year, setYear] = useState<number>(Number.isNaN(initialYear) ? 0 : initialYear);
    ...
    // The category input is also free-text — debounce it the same way
    // so every keystroke doesn't fire a new query.
    ...
    useEffect(() => {
        if (catDebounceRef.current !== null) {
    ...
    // A change to the query, kind, or new filters resets pagination
    // to page 1 so a new search doesn't drop the reader on page 4 of
    // the previous one. We do this only when the input actually
    // differs from the URL — an in-flight ``page=2`` navigation from
    // the pagination buttons should not immediately snap back.
    useEffect(() => {
        setPage(1);
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [debouncedQ, kind, year, debouncedCategory]);
    ...
    useEffect(() => {
        ...
        if (year && year > 0) next.set('year', String(year));
        if (debouncedCategory) next.set('category', debouncedCategory);
        if (page > 1) next.set('page', String(page));
        setParams(next, { replace: true });
    }, [debouncedQ, kind, year, debouncedCategory, page, setParams]);
    ...
    }, [debouncedQ, kind, year, debouncedCategory, page]);
        ...
        <div className="min-h-screen flex flex-col bg-gray-50">
            ...
            <main className="flex-1 py-12">
                <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
                    {/*
                     * Filter chips: Year (last 5) + free-text Category.
                     * Rendered above the results so the reader can
                     * narrow after seeing an over-broad first cut.
                     */}
                    <div className="mb-6 flex flex-wrap items-center gap-2">
                        <label className="inline-flex items-center gap-2 text-xs font-semibold text-gray-600">
                            ...
                            <select
                                ...
                                className="rounded-full border border-gray-200 bg-white px-3 py-1 text-xs font-semibold text-gray-800 focus:ring-2 focus:ring-brand-400"
                            >
                        ...
                        <label className="inline-flex items-center gap-2 text-xs font-semibold text-gray-600">
                            ...
                            <input
                                ...
                                className="w-44 rounded-full border border-gray-200 bg-white px-3 py-1 text-xs font-semibold text-gray-800 focus:ring-2 focus:ring-brand-400 placeholder:text-gray-400 placeholder:font-normal"
                            />
                        </label>
                        {(year > 0 || category) && (
                            <button
                                ...
                                className="rounded-full border border-gray-200 bg-white px-3 py-1 text-xs font-semibold text-gray-500 hover:text-brand-700 hover:border-brand-200"
                            >
                                Clear filters
                            </button>
                        )}
                    ...
                    {!trimmedQ ? (
                        ...
                        <>
                            ...
                            <ul className="space-y-5">
                                {items.map((a) => {
                                    const highlightHtml = a.highlighted
                                        ? sanitiseHighlight(a.highlighted)
                                        : '';
                                    return (
                                        <li
                                            key={a.id}
                                        ...
                                        >
                                            ...
                                            >
                                                ...
                                                {highlightHtml ? (
                                                    <p
                                                        className="mt-3 text-gray-700 leading-relaxed [&_mark]:bg-yellow-100 [&_mark]:text-gray-900 [&_mark]:font-semibold [&_mark]:rounded [&_mark]:px-0.5"
                                                        // Content is a ts_headline HTML fragment.
                                                        // We whitelist only <mark>/</mark> above
                                                        // before rendering.
